  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-soft relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-6 mb-8 relative z-10 border-b border-slate-100 pb-8">
        <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-amber-500/30 ring-4 ring-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8 relative z-10">
        {items.map(([l, v]) => (
          <div key={l} className="group">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1 group-hover:text-amber-500 transition-colors">{l}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          {label:'Total Pertandingan', value:list.length, icon: '📋', color:'from-blue-50 to-indigo-50 text-blue-700 border-blue-100'},
          {label:'Mendatang', value:mendatang.length, icon: '⏳', color:'from-amber-50 to-yellow-50 text-amber-700 border-amber-100'},
          {label:'Selesai', value:selesai.length, icon: '✅', color:'from-emerald-50 to-teal-50 text-emerald-700 border-emerald-100'},
        ].map(s => (
          <div key={s.label} className={`p-5 border rounded-2xl text-center bg-gradient-to-br ${s.color} shadow-sm hover:-translate-y-1 transition-transform`}>
      ...
      {list.length === 0 ? (
        <div className="bg-white/50 backdrop-blur-sm rounded-3xl p-12 text-center border border-slate-100 shadow-soft">
          ...
          <h3 className="text-lg font-bold text-slate-900 mb-1">Belum Ada Pertandingan</h3>
          <p className="text-slate-500 text-sm">Tidak ada jadwal pertandingan di bulan ini.</p>
        </div>
      ) : (
            ...
            return (
              <div key={p.id} className="bg-white/80 backdrop-blur-xl p-5 sm:p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-soft transition-all duration-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 group">
                <div className="flex items-start gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-inner ${isUpcoming?'bg-gradient-to-br from-amber-400 to-orange-500':'bg-slate-200'}`}>
                    <svg className={`w-6 h-6 ${isUpcoming?'text-white':'text-slate-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9"/>
                    </svg>
                  </div>
                  <div>
                    <div className="flex items-center gap-2 flex-wrap mb-1.5">
                      <span className="font-extrabold text-slate-900 text-lg leading-snug">{p.nama_event}</span>
                      <span className={`text-[10px] px-2.5 py-1 rounded-full font-extrabold uppercase tracking-wider ${isUpcoming?'bg-amber-100 text-amber-700':'bg-slate-100 text-slate-500'}`}>
                    ...
                    <div className="flex flex-col gap-1">
                      {p.cabor_nama && <span className="text-xs font-bold text-brand-600 bg-brand-50 w-fit px-2 py-0.5 rounded">{p.cabor_nama}</span>}
                      {p.keterangan && <p className="text-xs text-slate-400 italic">{p.keterangan}</p>}
                    </div>
                  </div>
                </div>
                <div className="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0 justify-end">
                  <button onClick={()=>{setForm({nama_event:p.nama_event,tanggal:p.tanggal,lokasi:p.lokasi||'',cabor_id:p.cabor_id||'',keterangan:p.keterangan||''});setEditId(p.id);setModal(true)}}
                    className="p-2.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors shadow-sm bg-slate-50 border border-slate-100">
                  ...
                  <button onClick={()=>hapus(p.id)}
                    className="p-2.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors shadow-sm bg-slate-50 border border-slate-100">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 font-sans animate-fade-in">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 sm:p-8 animate-scale-in">
            <h3 className="text-xl font-extrabold text-slate-900 mb-6 flex items-center gap-2">
              ...
              {editId?'Edit':'Tambah'} Pertandingan
            </h3>
            <div className="space-y-4">
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Nama Event / Kejuaraan *</label><input className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all outline-none" value={form.nama_event} onChange={e=>setForm(f=>({...f,nama_event:e.target.value}))} placeholder="Contoh: Liga Futsal Banyumas 2024" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Tanggal *</label><input type="date" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all outline-none" value={form.tanggal} onChange={e=>setForm(f=>({...f,tanggal:e.target.value}))} /></div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Cabang Olahraga</label>
                  <select className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all outline-none appearance-none" value={form.cabor_id} onChange={e=>setForm(f=>({...f,cabor_id:e.target.value}))}>
                    <option value="">-- Pilih --</option>
                    {cabor.map(c=><option key={c.id} value={c.id}>{c.nama}</option>)}
                  </select>
                </div>
              </div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Lokasi</label><input className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all outline-none" value={form.lokasi} onChange={e=>setForm(f=>({...f,lokasi:e.target.value}))} /></div>
              <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Keterangan</label><textarea className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all outline-none resize-none" rows={2} value={form.keterangan} onChange={e=>setForm(f=>({...f,keterangan:e.target.value}))} /></div>
            </div>
            <div className="flex justify-end gap-3 mt-8">
              ...
              <button className="px-6 py-2.5 text-sm font-bold text-white bg-brand-600 hover:bg-brand-700 rounded-xl shadow-lg shadow-brand-500/30 transition-all" onClick={simpan} disabled={loading}>{loading?'Menyimpan...':'Simpan Pertandingan'}</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div className="space-y-8">
      <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-soft">
        <h3 className="font-extrabold text-slate-900 text-lg mb-6 flex items-center gap-2">
          ...
          Upload Sertifikat / Lisensi Wasit
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
          <div className="md:col-span-2"><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Judul Dokumen *</label><input className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all outline-none" value={form.judul} onChange={e=>setForm(f=>({...f,judul:e.target.value}))} placeholder="Contoh: Lisensi Wasit Nasional C1" /></div>
          <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Tipe Dokumen</label>
          ...
          <div><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">File (JPG/PNG/PDF) *</label><input type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm file:mr-4 file:py-1 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-brand-50 file:text-brand-700 hover:file:bg-brand-100 transition-all cursor-pointer outline-none" onChange={e=>setFile(e.target.files[0])} /></div>
          <div className="md:col-span-2"><label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Keterangan Tambahan</label><input className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-all outline-none" value={form.keterangan} onChange={e=>setForm(f=>({...f,keterangan:e.target.value}))} placeholder="Opsional" /></div>
        </div>
        <button className="w-full sm:w-auto bg-brand-600 hover:bg-brand-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-brand-500/30 transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:hover:translate-y-0" onClick={handleUpload} disabled={loading}>{loading?'Mengupload...':'Simpan Dokumen'}</button>
      </div>
      <div>
        <h3 className="font-extrabold text-slate-900 text-lg mb-4 ml-1">Koleksi Dokumen & Lisensi</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
          {list.length===0 ? <div className="col-span-full bg-white/50 backdrop-blur-sm rounded-2xl text-center text-slate-400 py-12 border border-slate-100 border-dashed">Belum ada dokumen tersimpan.</div>
            : list.map(s=>(
...
export default function WasitDashboard() {
  ...
  const tabs = [
    ...
    {id:'pertandingan',label:'Pertandingan', icon: '🏁'},
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      <header className="bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-slate-100 px-4 sm:px-8 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            <p className="font-extrabold text-slate-900 text-sm sm:text-base leading-none">Dashboard Wasit</p>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-3 text-right">
            <div>
              ...
              <p className="text-xs font-medium text-brand-600 mt-1">Wasit/Juri</p>
            </div>
          ...
          <button onClick={()=>{logout();navigate('/login')}} className="text-sm font-bold text-rose-500 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 px-4 py-2 rounded-xl transition-colors">Keluar</button>
        </div>
      </header>
        ...
        <div className="mb-8">
          ...
          {profil?.cabor_nama && (
            <p className="text-slate-500 font-medium mt-2 flex items-center gap-2">
              ...
              Wasit {profil.cabor_nama} <span className="mx-1">•</span> Lisensi {profil.lisensi||'-'}
            </p>
          )}
        </div>
