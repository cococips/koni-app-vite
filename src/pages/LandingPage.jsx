                ...
                <Link key={b.id} to={`/berita/${b.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${i === 0 ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}>
                        {b.kategori}
                      </span>
                      <span className="text-xs text-gray-400">{formatTanggal(b.created_at)}</span>
                    </div>
                    <h3 className={`font-bold text-sm leading-snug group-hover:text-red-600 transition-colors ${i === 0 ? 'text-red-800' : 'text-gray-800'}`}>
                      {b.judul}
                    </h3>
